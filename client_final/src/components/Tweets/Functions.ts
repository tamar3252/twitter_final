...
export const getYourTweets = (): allTweetsQuery => {
    const useYourTweetsQuery = (): allTweetsQuery => {
        return useQuery<Tweet[]>('yourTweets',
            async (): Promise<Tweet[]> => {
                const response: Response = await fetch(`${process.env.REACT_APP_SERVER_URL}/tweet/your_tweets`, {
                    method: 'GET',
                    headers: {
                        'Content-Type': 'application/json',
                        'Authorization': `${Cookies.get('token')}`
